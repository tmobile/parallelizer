...
// Run is the method that will be called to actually perform the work.
...
// embedded in the type, must be accessed in a thread-safe fashion.
//
...
// Integrate is used to process the data returned by a Run method.  It
// is passed a Worker object, which it may use to make additional
// calls.  All instances of Integrate operate in a single goroutine,
// the manager goroutine of the Worker, and so must not block; a
// side-effect is that the elements they interact with may be safely
// accessed without concern for parallel calls to Integrate.  The idea
// of Integrate is to allow the results from the various Run method
// calls to be combined together into a single result, which may then
...
// Result is called by the Worker.Wait method once all the worker
// goroutines have been terminated.  It is intended to work in
// conjunction with Integrate to enable the final result of the work
// to be reported to the worker of Worker.Wait.  It runs in the same
// goroutine as Worker.Wait, and need not worry about any other
...
// the parallelizer has been shut down through a call to Wait.
...
// Wait is called to shut down the parallelizer and return the
// final result; it will block the worker until all data has
// been processed and all worker goroutines have stopped.
// Note that the final result, generated by Runner.Result, is
// saved by Worker to satisfy later calls to Wait.  If Wait is
